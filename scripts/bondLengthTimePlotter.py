# Outdated equations for expected length etc. Find them in forceExtension.py
...
# Constants
timestep = 0.00001  # lj
nevery = 1 # how often data is dumped
...
def main():
    ...
    #plt.plot(blen_arr[0,:])
    plt.title(f'Bond 1 Length / Time at {data_df['force'].iloc[0]} Force (dimensionless)')
    plt.xlabel('time (s)')
    plt.ylabel('length (dimensionless)')
    cbar = plt.colorbar(sc)
    cbar.set_label('Type')
    plt.savefig('Bond 1 Length Against Time.eps', bbox_inches='tight')
    plt.savefig('Bond 1 Length Against Time.png', bbox_inches='tight')
    ...
    #plt.plot(blen_arr[1,:])
    plt.title(f'Bond 2 Length / Time at {data_df['force'].iloc[0]} Force (dimensionless)')
    plt.xlabel('time (s)')
    plt.ylabel('length (dimensionless)')
    cbar = plt.colorbar(sc)
    cbar.set_label('Type')
    plt.savefig('Bond 2 Length Against Time.eps', bbox_inches='tight')
    plt.savefig('Bond 2 Length Against Time.png', bbox_inches='tight')
    ...
    plt.title(f'Monomer Length / Time at {data_df['force'].iloc[0]} Force (dimensionless)')
    plt.xlabel('time (s)')
    plt.ylabel('length (dimensionless)')
    cbar = plt.colorbar(sc)
    cbar.set_label('Type')
    plt.savefig('Monomer Length Against Time.eps', bbox_inches='tight')
    plt.savefig('Monomer Length Against Time.png', bbox_inches='tight')
