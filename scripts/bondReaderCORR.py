...
def main():
    ...
    r = np.array([Zr_bond(i, 45) for i in x])
    Z, err = integrate.quad(Zr_bond, 0, np.inf, args=45)
    plt.hist(data_df[(data_df['K_r phys'] == 45) & (data_df['K_theta phys'] == 20.0)]['bond lengths'].iloc[0][0,:], bins=200, density=True, alpha=0.5, label='k_r = 45 lj')
    plt.plot(x, r/Z, label='k_r = 45 lj theory')
    r = np.array([Zr_bond(i, 5) for i in x])
    Z, err = integrate.quad(Zr_bond, 0, np.inf, args=5)
    plt.hist(data_df[(data_df['K_r phys'] == 5) & (data_df['K_theta phys'] == 20.0)]['bond lengths'].iloc[0][0,:], bins=200, density=True, alpha=0.5, label='k_r = 5 lj')
    plt.plot(x, r/Z, label='k_r = 5 lj theory')
    plt.xlabel('Bond length (lj)')
    plt.ylabel('Frequency')

    plt.figure(7)
    plt.title('Monomer angle histogram at k_r = 45 lj. Timestep=0.001, Count=800000, 5 seeds')
    x = np.linspace(0,np.pi,200)
    r = np.array([ZTheta(i, 20) for i in x])
    Z, err = integrate.quad(ZTheta, 0, np.pi, args=20)
    plt.hist(monomer_df[(monomer_df['K_r'] == 45) & (monomer_df['K_theta'] == 20.0)]['angles'].iloc[0]*np.pi/180, bins=200, density=True, alpha=0.5, label='k_theta = 20 lj')
    plt.plot(x, r/Z, label='k_theta = 20 lj theory')
    r = np.array([ZTheta(i, 2) for i in x])
    Z, err = integrate.quad(ZTheta, 0, np.pi, args=2)
    plt.hist(monomer_df[(monomer_df['K_r'] == 45) & (monomer_df['K_theta'] == 2.0)]['angles'].iloc[0]*np.pi/180, bins=200, density=True, alpha=0.5, label='k_theta = 2.0 lj')
