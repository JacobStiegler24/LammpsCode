# Plot force extension over multiple runs against theory. Folded, unfolded, reacting all allowed, change path in readfile() accordingly.
...
def readfile():
    ...
    path = os.path.join(currentdir, r'output/ForceSeedUF')
...
def main():
    ...
    F1 = np.linspace(0,np.max(monomer_df['force']),20)
    # x_FF = np.array([Expected_r_FF(i) for i in F1])
    x_UF = np.array([Expected_r_UF(i) for i in F1])
    # F2 = np.linspace(0,np.max(monomer_df['force'])+0.1,40)
    ...
    # plt.figure(1)
    # plt.title('Monomer States Force-Extension Curve')
    # plt.ylim(-3,55)
    # plt.xlim(1.9,4.8)
    # plt.xlabel('Monomer Length (lj)')
    # plt.ylabel('Force (lj)')
    # plt.plot(x_UU, F2, label='UU Theory')
    # plt.plot(x_UF, F1, label='FU Theory')
    ...
    # plt.legend()
    # plt.savefig('monomer_force_extension_theory.png', bbox_inches='tight')
    # plt.savefig('monomer_force_extension_theory.eps', bbox_inches='tight')

    plt.figure(2)
    plt.title('UF Monomer States Force-Extension')
    plt.ylim(np.min(monomer_df['force'])-2,np.max(monomer_df['force'])+2)
    plt.xlim(np.min(monomer_df['monomer length mean'])-0.1,np.max(monomer_df['monomer length mean'])+0.1)
    plt.xlabel('Monomer Length (lj)')
    plt.ylabel('Force (lj)')
    plt.plot(x_UF, F1, label='UF Theory')
    # plt.scatter(monomer_df['monomer length mean'], monomer_df['force'])
    plt.errorbar(monomer_df['monomer length mean'], monomer_df['force'], yerr=monomer_df['monomer length sem'], fmt='.', capsize=4, ecolor='black')
    # plt.plot(monomer_df['monomer length mean'], monomer_df['force'])
    plt.legend()
    plt.savefig('monomer_force_extension_UF.png', bbox_inches='tight')
    plt.savefig('monomer_force_extension_UF.eps', bbox_inches='tight')
